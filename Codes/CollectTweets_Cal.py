...
#            print ("Happy!! {}".format(data['text'].encode('utf-8')))
    ...
    def store_json(self,data,ttype):
        if ttype == "bought":
            with open('tweet_stream_bought_CA.json', 'a') as f:	
                json.dump(data, f)
                f.write("\n")
        elif ttype == "workout":
            with open('tweet_stream_workout_CA.json', 'a') as f:	
                json.dump(data, f)
                f.write("\n")
        elif ttype == "happy":
            with open('tweet_stream_happy_CA.json', 'a') as f:	
                json.dump(data, f)
                f.write("\n")
        elif ttype == "ride":
            with open('tweet_stream_ride_CA.json', 'a') as f:	
                json.dump(data, f)
                f.write("\n")
        elif ttype == "food":
            with open('tweet_stream_food_CA.json', 'a') as f:	
                json.dump(data, f)
                f.write("\n")
#    def sendemail(count): 
...
#        keyword = 'trump'
    ...
    for i in range (0,20):
        try:
            stream.statuses.filter(locations=[-124.47,32.09,-116.87,41.56])
        except:
